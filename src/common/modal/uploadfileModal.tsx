...
const getAcceptType = (formatType: string) => {
  switch (formatType) {
    case "PubMed Nbib Reference Format":
      return ".txt"; // Accept only .txt files
    case "Embase Full Text Search CSV":
      return ".xlsx"; // Accept only .xlsx files
    case "PubMed Email Alerts File":
      return ".eml"; // Accept only .eml files
    case "Custom":
      return ".xlsx"; // Accept only .xlsx files for Custom
    default:
      return ""; // Accept any file (optional, for no selection)
  }
};
    ...
    <div className={`modal ${isOpen ? "visible" : "hidden"} w-[550px]  p-4`}>
      <div className="text-14 cursor-pointer">
        ...
        <p className="font-Archivo text-violet text-Archivo">
          CoVigilAI supported file formats:-
        </p>
        <div className="relative">
          <select
            className="block cursor-pointer text-14 w-[400px] px-4 py-2 pr-8 text-sm leading-tight bg-white border border-gray-300 rounded-md appearance-none focus:outline-none focus:border-blue-500"
            onChange={(e) =>
              setFormValues({
                ...formValues,
          ...
          >
            ...
            <option value="PubMed Nbib Reference Format">
              PubMed RIS (.txt format)
            </option>
            <option value="Embase Full Text Search CSV">
              Embase export (.xlsx format)
            </option>
            {/* <option value="ProQuest Excel">ProQuest Excel</option> */}
            <option value="PubMed Email Alerts File">
              PubMed email alerts (.eml format)
            </option>
 <option value="PubMed Message Alerts File">

              PubMed email alerts (.msg format)
            </option>
  <option value="PubMed Ris Reference Format">
              Embase export (.ris format)
            </option>

            <option value="Custom">Custom (Embase / ProQuest / MEDLINE - .xlsx format)</option>
        ...
        <label className="mt-4 cursor-pointer text-14 text-base font-archivo font-medium capitalize rounded-md">
          <input
            type="file"
 accept={getAcceptType(formValues.selectedFormatType)}
 ...
 <div className="flex text-14 mb-2">
              ...
              <select
                className="block cursor-pointer text-14 w-[200px] px-4 py-2 pr-8 text-sm leading-tight bg-white border border-gray-300 rounded-md appearance-none focus:outline-none focus:border-blue-500"
                onChange={(e) => setCustomFields("PUI", e.target.value)}
              >
                <option>
                  <select value="" id="select">
                    Select
