...
const AddDuplicates: React.FC<AddDrugOfChoiceProps> = ({ handelClose }) => {
...
setIsLoading(true);
    ...
    try {
      const payload = {
        file_type: "Duplicate Of Choice File",
        formData: formData,
      };
      const response = await dispatch(CreateDeduplicateAsync(payload));
      
      if (CreateDeduplicateAsync.fulfilled.match(response)) {
        await dispatch(GetDeduplicateAsync());
        Toast(systemMessage.DeduplicateAddSuccessfully, {
          type: "success",
        });
