...
import {
  ...
  AbstractReviewMonitorAOITotalRecordIdAsync,
  AssignToAsync,
  GetTeamUserAsync,
...
const ArticleOfInterestICSR: React.FC<IProps> = ({ monitor_id, label }) => {
  ...
  const {
    ...
    monitorDetail,
    AbstractReviewMonitorAOITotalRecord,
  ...
  useEffect(() => {
    if (status === STATUS.fulfilled) {
      ...
      setTotalRecords(AbstractReviewMonitorAOITotalRecord);
    } else {
  ...
  useEffect(() => {
    const fetchData = async () => {
      if (defaultPage !== 0) {
        ...
        dispatch(
          AbstractReviewMonitorAOITotalRecordIdAsync(monitor_id)
        );
        setIsLoading(false);
      }
    };
                              ...
                              item.ai_tags.Medications.map(
                                (medication, index: number) => {
                                  const entitiesToShow =
                                    medication.entity.slice(0, 5); // Show the first 5 entities directly
...
export default ArticleOfInterestICSR;
