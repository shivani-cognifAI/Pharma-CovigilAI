...
useEffect(() => {
 ...
 <div className=" absolute text-14 right-60 p-2  rounded-md mt-12 ">
 ...
 mandatoryHeaders = {["Article Id","Title"]}
   mandatoryBodyKeys = {["article_id","title"]}
                  ...
                  data={data}
                  headers={headers}
                  fileNamePrefix="Invalid ICSR List"
                  fileSavedName={`${
                    Utils.getUserData()?.user_name
                  }_InValidICSRList_${Utils.getCurrentDateAndTime()}`}
                  disable={false}
                  searchRow={undefined}
                  startRow={undefined}
                  endRow={undefined}
            ...
            <table className="w-full text-14 border border-collapse table-auto">
              <thead className="border-style text-sm text-center">
                <tr className="font-Archivo cursor-pointer header-style capitalize text-style text-sm bg-gray-50">
                  ...
                  <th className="px-2 py-2 w-20   hover-text-style text-center">
                    ID
                  </th>
                  <th className="px-2 py-2 w-40   hover-text-style text-center">
                    Title{" "}
                  </th>
                  <th className="px-2 py-4 w-32  hover-text-style">AI Tags</th>
                  <th className="px-2 py-4 w-32  hover-text-style">
                    Medications
                  </th>
                  <th className="px-2 py-4 w-32  hover-text-style">
                    Designated Medical Events
                  </th>
                  <th className="px-2 py-4 w-24  hover-text-style">
                  ...
                  <th className="px-2 py-4 w-20  hover-text-style">
                    Assign To
                  </th>
                  <th className="px-2 py-4 w-20  hover-text-style">Status</th>
                  <th className="px-2 py-4 w-20  hover-text-style">
                    Updated By
                  </th>
                </tr>
              ...
              <tbody>
                {sortedData &&
                  sortedData
                    ...
                    .map((item, index) => (
                      <tr
                        className="border-b cursor-pointer text-right text-sm border-style hover:bg-ghostwhite mb-2"
                        key={index}
                        onClick={() => {
                      ...
                      >
                        ...
                        <td className="px-2 w-24 text-center items-center">
                          <div className="mt-1">
                            {item?.article_id ? item?.article_id : "-"}
                          </div>
                        </td>
                        <td className="px-2 text-center w-20">
    ...
    {!(item?.tags && Object.keys(item.tags).length > 0) &&
      ...
      !item?.adverse_reaction && (
        <div className={`text-center`}>-</div>
                        ...
                        <td className="px-2 w-40 text-dimgray text-center capitalize">
<>
  {/* Render drug_of_choice */}
  {item?.drug_of_choice && item.drug_of_choice.length > 0 ? (
    // Use filter to create a unique array without Set
  ...
  {item?.ai_tags?.Medications ? (
    item.ai_tags.Medications.map((medication, index: number) => {
      const entitiesToShow = medication.entity.slice(0, 5); // Show the first 5 entities directly

      // Create a normalized drug_of_choice array for quick lookup
      const drugOfChoiceNormalized = item.drug_of_choice?.map(choice => choice.toLowerCase()) || [];

      // Filter out medications that are already in drug_of_choice (normalize to lowercase)
      const uniqueEntities = entitiesToShow.filter(entity => 
        entity.length > 0 && !drugOfChoiceNormalized.includes(entity.toLowerCase())
      );

      // Filter unique medications using filter method
      ...
      return uniqueMedicationEntities.length > 0 ? (
        <div key={index}>
          {/* Directly map uniqueMedicationEntities */}
    ...
    <div className={`text-center`}></div>
                        ...
                        <td className="px-2 py-4 text-center capitalize">
                          <div className="w-40">
                            ...
                            {/* commenting it out for hiding the cofidence score */}
                            {/* <div className="mt-2 table-date-font text-dimgray">
