...
const ValidICSR: React.FC<IProps> = ({ monitor_id, label }) => {
  ...
  const [activeTab, setActiveTab] = useState<string>(label);
  const {
    status,
    teamUsers,
  ...
  useEffect(() => {
    // Update the active tab based on the label
    setActiveTab(label);
  }, [label]);

  useEffect(() => {
    if (status === STATUS.fulfilled && activeTab === "Valid ICSR") {
   ...
   * @param filteredData The array of objects to be sorted.
   * @param sortConfig Configuration object containing sorting key and direction.
            ...
            <div className="mr-4 cursor-pointer justify-end flex">
              ...
              <div className="absolute">
                <ExportDropdown
                  ...
                  data={data}
                  headers={headers}
                  fileNamePrefix="Abstract Valid ICSR List"
                  fileSavedName={`${
                    Utils.getUserData()?.user_name
                  }_AbstractValidICSRList${Utils.getCurrentDateAndTime()}`}
                  disable={false}
                  searchRow={""}
                  startRow={""}
                  endRow={""}
                ...
                <tr className="font-Archivo cursor-pointer header-style capitalize text-style text-sm bg-gray-50">
                  ...
                  <th className="px-2 py-2 w-40 text-center">
                    <div className="relative">
                      <span className="flex hover-text-style title-width text-center items-end  cursor-pointer px-2 py-2">
                        Title{" "}
                      </span>
                    </div>
                  </th>
                ...
                {sortedData &&
                  sortedData
                    ...
                    .map((item, index) => (
                      ...
                      >
                        <td className="cursor-pointer text-center title-width">
                          <div>
                            <input
                        ...
                        <td className="px-2 w-40 text-center">
                          ...
                          <div className="flex">
                            <div className="mt-2 text-center table-date-font text-dimgray">
                              Published on:{" "}
                              {item?.updated_on
