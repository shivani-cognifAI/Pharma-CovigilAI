...
import {
  ...
  AssignToAsync,
  getRandomSamplingValueAsync,
  ReviewQcSendMailAsync,
  SumbitRandomSamplingDataAsync,
...
const [isRandomSamplingDone, setIsRandomSamplingDone] = useState(false);
  ...
  const [randomSampling, setRandomSampling] = useState<string | null>(null)


  const { advanceReviewMonitor, AdvanceReviewMonitorInValidICSRTotalRecord,RandomSamplingValue } =
...
const [tempSamplingPercentage, setTempSamplingPercentage] = useState(0);
 ...
 const fetchData = async () => {
  ...
  const previousMonitorIdRef = useRef(""); // Store the previous monitor_id

useEffect(() => {
  const fetchRandomSamplingValue = async () => {
    if (!monitor_id) return; 
    if (previousMonitorIdRef.current !== monitor_id) {
      setIsLoading(true); // Start loading state
      try {
        await dispatch(getRandomSamplingValueAsync(monitor_id));
       setRandomSampling(RandomSamplingValue)
      } catch (error) {
        console.error("Error fetching random sampling value:", error);
      } finally {
        setIsLoading(false);
      }
    }
    previousMonitorIdRef.current = monitor_id;
  };

  fetchRandomSamplingValue();
}, [monitor_id, dispatch]); 
...
useEffect(() => {
  ...
  const handleSamplingPercentageChange = async() => {
    
 try {
      setIsLoading(true);
      const payload = {
       monitor_id: monitor_id,
        sampling: tempSamplingPercentage,
        
      };
      const res = await dispatch(SumbitRandomSamplingDataAsync(payload));
      if (SumbitRandomSamplingDataAsync.fulfilled.match(res)) {
        setIsLoading(false);
        if (res.payload.status === 200) {
          Toast(systemMessage.SendRandomSamplingSuccess, { type: "success" }); 
      const payload = {
      monitor_id,
      pageNumber: 1,
      perPage: defaultPage,
    ...
    dispatch(AdvanceReviewMonitorInVaildICSRTotalRecordIdAsync(monitor_id));
    dispatch(AdvanceReviewMonitorIdAsync(payload)); 
dispatch(getRandomSamplingValueAsync(monitor_id))
    setRandomSampling(RandomSamplingValue)
...
// console.log("current randomSampling",randomSampling)
  return (
    <React.Fragment>
      <div>
...
<>
      ...
      <div className="mt-5 ml-5 ">Random Sampling:</div>
      {randomSampling !== null ? (
        <div className="mt-5 ml-2 text-17 font-medium">
          {randomSampling}%
        </div>
      ) : (
        <>
          <div className="w-[8%] relative mt-3">
            <select
              className="block mb-2 text-14 cursor-pointer w-[95%] py-2 text-sm text-black leading-tight bg-white border rounded-md appearance-none focus:outline-none focus:border-blue-500"
              onChange={(e) => setTempSamplingPercentage(parseInt(e.target.value))}
              value={tempSamplingPercentage}
            >
          ...
          <div className="mt-3">
            <button
              ...
              disabled={!tempSamplingPercentage}
              onClick={handleSamplingPercentageChange}
            >
              Apply
            </button>
          </div>
...
{/* Render tags if present */}
    {item && item.tags && Object.keys(item.tags).length > 0 ? (
      Object.keys(item.tags).map((tag: string, index: number) => {
        const entities = (item.tags as Record<string, any>)[tag]?.[0]?.entity;
        return (
          <div
            title={entities}
            key={index}
            className={`text-center px-3 flex-wrap mr-2 mb-1 py-1 ${getClass(tag)}`}
          >
            {tag}
          </div>
        );
