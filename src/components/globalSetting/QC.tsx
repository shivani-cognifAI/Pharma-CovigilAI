...
const QC: React.FC<QCProps> = ({ mandatoryHeaders = [] }) => {
  ...
  useEffect(() => {
    const fetchHeaders = async () => {
      try {
        ...
        const headersFromAPI: string[] = data?.qc_review.map(
          (item: APIResponseItem) => item.Value
        );
        const keysFromAPI: string[] = data?.qc_review.map(
          (item: APIResponseItem) => item.key
        );
        const activeHeaders: string[] = data?.qc_review
  ...
  const handleSave = async () => {
    try {
      ...
      const updatedQCReview = tableBodyKeys.map((key, index) => {
      ...
      const updatedPayload = {
        ...
        qc_review: updatedQCReview,
      };

      await dispatch(GeneralSettingAsync(updatedPayload));
      Toast("QC Review export preference set successfully.", { type: "success" });
    } catch (error) {
