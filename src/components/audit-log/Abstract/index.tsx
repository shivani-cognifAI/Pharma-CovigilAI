...
const Abstract: React.FC<IProps> = ({ selectedItems, label, MonitorName,onclose}) => {
...
const payload = {
  ...
  const handleDownload = () => {
    ...
    const data = [
      ...
      // ["Confidence Score", selectedItems.confidence_score || "-"], // Uncomment if needed
    ...
    const csvContent = historyData
      ...
      .map((item) => [
        ...
        item.to_decision || "-",
        item.to_comments || "-",
        item.to_categories ? item.to_categories.join(", ") : "-",
        item.to_classifications ? item.to_classifications.join(", ") : "-",
      ]);
    ...
    exportToExcel(
      ...
      }_Abstract_auditlog_${Utils.getCurrentDateAndTime()}.xls`
    );
